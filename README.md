...
The following is a first go at building out a simulated multi data center Posgres 16 on Ubuntu 20.04 using Patroni, etcd and Docker. Optionally, pgbackrest as a backup solution as well.

There is no haproxy in this environment. The use of libpq and it's new random feature works great.

This will spin up 5 etcd containers and 6 postgres container. Prior to starting the containers, we create 3 separate networks where a pair of postgres containers will run in a given network. This is simply to simulate multi data centers. 

The etcd folder contains the Docker file necessary to create the image for the etcd builds. Since the ubunto package does not have the latest version of etcd ( as of 4/30/24 ), we use the latest from google to build our image.

The pgpatroni folder contains the Docker file necsessary to create an Ubunto 20.04 image running Postgresql 16 and Patroni. 

The pgbackrest folder has the necessry files to create the pgbackrest container so we can backup the database.
...
The pg containers have two volumes defined, /pgxdata and /pgxhome. /pgxdata is for keeping the data base even if we remove the container and /pgxhome is for keeping postgres related info.
...
You could run ```docker-compose up``` but that leaves the terminbal occupied while running the containers
...
P{orts are exposed and assigned, so you can always use the same port to copnnect to a spcific db instance from outside the container

```psql -h localhost -p 50554 -U postgres```

Would grant youi psql access to the pgha-pg2-node2 container.
...
#### Want to see wich server is the primary server?
...
Based on opur patronictl list output, we know that pg1 is the leader. So, with the following connection string we can specify all our pg hosts and connect to the primary frominside the container.
...
We ran the above command as user root but execcuted it as postgres.  We could ```su - postgres``` and run

```pgbackrest --stanza=${STANZA_NAME} stanza-create```

you get some output andat the end you should see
...
Now its time to make the changes to the DCS
...
To see you rbackup info just run 
